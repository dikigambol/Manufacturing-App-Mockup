...
export const getSidebarData = (lineId) => {
  ...
  // Separate main navigation and footer navigation
  const allNavGroups = lineNavGroups[lineId] || defaultNavGroups;
  const mainNavGroups = allNavGroups.filter(group =>
    !['Master Data', 'Management'].includes(group.title)
  );
  const footerNavGroups = allNavGroups.filter(group =>
    ['Master Data', 'Management'].includes(group.title)
  );
